...
uint32_t rel, dead;
int error_count ;
#define t1 250 //used for button check.
...
begin  
	initialize();
	while(1) {
		if (~PINB & (1<<PINB0))//Check if PORT is connected to Ground
		{ 
			//PORTB &= ~(1<<PORTB5);//OFF LED - Test mode
			authorized = AUTHORIZED;
		}else{
			//PORTB |= (1<<PORTB5);//ON LED - Normal Mode
			authorized = UNAUTHORIZED;
		}
		if (~PINB & (1<<PINB1)){
			//PORTB &= ~(1<<PORTB6); 
			key_or_lock = BASE_LOCK;
		}else{
			//PORTB |= (1<<PORTB6);
			key_or_lock = KEY;
		}
...
void IR_key(void* args) 
{
	...
	while(1)
	{
        if ((key_or_lock == KEY) || (test_mode == TEST))
	    {
			...
			else if (state_key == CHALLENGE) {
				char ir_rx_data[buffer_size];			    
				char rx_status;
				trtWait(SEM_RECV_IR);
				rx_status = ir_rec_packet(lock_tx_id, ir_rx_data);
				trtSignal(SEM_RECV_IR);
				if (!rx_status) {
					fprintf(&uart0, "Key Receives Challenge=%s\n\r", ir_rx_data);
					challenge_time_string = trtCurrentTime();
					state_key = RESPONSE;
				}else{
					fprintf(&uart0, "Key Challenge Packet NOT RECEIVED")
					//state_key = IDLE;
				}
			}
			else if (state_key == RESPONSE) {
				trtWait(SEM_SEND_IR);
...
void IR_lock(void* args)  
{
	...
	//fprintf(stdout,"\n\r In Lock\n\r\n\r");
	while(1) {
		if ((key_or_lock == BASE_LOCK) || (test_mode == TEST)) 
		{
			//PORTB |= (1<<PORTB6);
			if (state_lock == IDLE) 
			{
				...
				if ((!rx_status) || (ir_rx_data[0] == key_request_id))
				{
					fprintf(&uart0, "IN_CHALLENGE=%s\n\r", ir_rx_data);
					state_lock = CHALLENGE;
				}
			}
			else if (state_lock == CHALLENGE) { //base-lock sends challenge to key
				trtWait(SEM_SEND_IR);
				...
				fscanf(ir_rx_data, "%c%c%ld", l_command, key_id, challenge_time);//&ir_rx_data);
				if (!rx_status) {
					if (key_id == AUTHORIZED && (trtCurrentTime - challenge_time < 15625)) { // 1 second
						PORTD |= 0x04; //unlock
					}
				}
			}*/
			//else { state_lock == IDLE; }
		}
	rel = trtCurrentTime() + SECONDS2TICKS(0.09);
	dead = trtCurrentTime() + SECONDS2TICKS(0.1);
	trtSleepUntil(rel, dead);
...
begin  
	...
	// transmit
	sprintf(ir_tx_data,"%ld: %u, %u, %u, %u, %u", trtCurrentTime(), test_mode, key_or_lock, authorized, lock_command, unlock_command); //
...
begin  
	...
	if (authorized == AUTHORIZED) {
		sprintf(ir_tx_data, "%c>%s", lock_command, time_string);
	}
	else {
		sprintf(ir_tx_data, "%c<%s", lock_command, time_string);
	}
...
begin
	...
	if (!rx_status) {
		//ir_rx_data[6] = 0;
		fprintf(&uart0, "KEY_RECV_CHALLENGE=%s\n\r", ir_rx_data);
	}
	else {
		fprintf(&uart0, "%d%s\n\r", rx_status, ir_rx_data);
	}
...
begin
	...
	if (!rx_status) {
		//ir_rx_data[6] = 0;
		fprintf(&uart0, "LOCK_RECV_TASK_PAYLOAD=%s\n\r", ir_rx_data);
	}
	else {
		fprintf(&uart0, "%d%s\n\r", rx_status, ir_rx_data);
	}
...
begin
  ...
  //set up the ports
  DDRB=0xE0;    // PORTB 11100000 Outputs/Inputs
  PORTB=0x18;//1F;   // Out   00011000 
  time1=t1;
...
int main(void) {
  ...
  trtCreateSemaphore(SEM_RECV_IR, 1) ; // message receive interlock
 ...
 // --- creat tasks  ----------------
  ...
  //trtCreateTask(led2, 300, SECONDS2TICKS(0.1), SECONDS2TICKS(0.2), &(args[1]));
  ...
  trtCreateTask(check_button, 1000, SECONDS2TICKS(0.1), SECONDS2TICKS(0.2), &(args[2]));
