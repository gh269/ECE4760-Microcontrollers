...
void initialize(void) {
	...
	adc_init();
	// init PID control
	pid_Init(PROP, INTEG, DIFF, &pid_data)
    // ********************
    //init LED screens for visual feedback
	init_screens();
...
void adjustTemperature(void* args) {
	...
	while(TRUE){
		...
		cTemp = (adc_in + 3) / 2.1;
		if (cTemp < 0) cTemp = 0; 
		if (pid_Controller, dTemp, cTemp, &pid_data) {	// Factor of .95 to account for carryover effect
	...
	while(TRUE){
		...
		switch ( current_state){
		  ...
		  case STATE_HAPPY		   : if( go_switched(ant) ) next_state = STATE_GO;
			...
			case STATE_CURR_TEMP_COOK: if( !go_switched(ant) ) next_state = STATE_HAPPY;
									   else if( disp_switched(ant) == TIME_DISP) next_state = STATE_TIME_REM;
									   else if( time_rem > 0) next_state = STATE_CURR_TEMP_COOK;
